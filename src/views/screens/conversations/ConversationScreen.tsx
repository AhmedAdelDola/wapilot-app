...
import { ActionTabs, BottomSheetBackdrop, BottomSheetWrapper } from '@/views/components';
...
import {
  ...
  setBottomSheetState,
} from '@/viewmodels/store/conversation/conversationHeaderSlice';
import { resetActionState } from '@/viewmodels/store/conversation/conversationActionSlice';
...
import { selectFilters, FilterState } from '@/viewmodels/store/conversation/conversationFilterSlice';
import { ConversationPayload } from '@/viewmodels/store/conversation/conversationTypes';
...
import { clearAllContacts } from '@/viewmodels/store/contact/contactSlice';
import { clearAssignableAgents } from '@/viewmodels/store/assignable-agent/assignableAgentSlice';
import { profileService, LifecycleStage } from '@/models/services/profileService';

import i18n from '@/i18n';
import ActionBottomSheet from '@/views/navigation/tabs/ActionBottomSheet';
...
const ConversationList = () => {
  ...
  const clearAndFetchConversations = useCallback(async (filters: FilterState) => {
    ...
    await fetchConversations(filters, 1);
    fetchConversations(filters, 2);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const onChangePageNumber = () => {
    setPageNumber(prev => {
      ...
      fetchConversations(filters, nextPage);
      return nextPage;
    });
  };
