...
const ChatMessageItem = React.memo(({
  m, idx, showDateHeader, isActivity, isPrivateMsg, isOutgoing, time, msgTextColor, messageText,
  isDark, name, conversation, messageMap, currentUserId, currentUserThumbnail, conversationId,
  highlightedMessageId, scrollToMessage, setQuotedMessage, setFileViewer, handleOpenAttachment,
  dispatch, showToast, isArabic, onLayoutMessage,
}: any) => {
  return (
    <>
      {showDateHeader && (
      ...
      {!isActivity && isPrivateMsg && (
        <View style={{ width: '100%', flexDirection: 'row', justifyContent: 'flex-end', marginTop: 10, marginBottom: 2, paddingHorizontal: 8 }}>
          <View
            style={{
            ...
            {messageText ? (
              <Text
                style={{
                  ...
                  fontSize: 15,
                  lineHeight: 22,
                  textAlign: isArabicText(messageText) ? 'right' : 'left',
                }}>
                {messageText}
              </Text>
            ) : null}
      ...
      {!isActivity && !isPrivateMsg && (
        <View
          onLayout={onLayoutMessage ? ({ nativeEvent }) => onLayoutMessage(m.id, nativeEvent.layout.y) : undefined}
          style={{
            width: '100%',
            flexDirection: 'row',
              ...
              <View
                ...
                <ChatDeliveryStatus
                  ...
                  onRetry={() => {
                    ...
                    dispatch(conversationActions.sendMessage({ conversationId, message: messageText, private: false, sender: { id: currentUserId!, thumbnail: currentUserThumbnail } }));
                  }}
                />
              </View>
            ) : null}
