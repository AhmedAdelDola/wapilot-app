...
class ContactService {
  ...
  async getContact(contactId: number, includeContactInboxes: boolean = true): Promise<Contact> {
    ...
    // TEMP LOG: inspect contact response shape (esp. lifecycle stage)
    console.log('[TEMP] getContact contact keys:', JSON.stringify(Object.keys(contact)));
    console.log('[TEMP] getContact custom_attributes:', JSON.stringify(contact?.custom_attributes));
    console.log('[TEMP] getContact lifecycle_stage_id:', (contact as any)?.lifecycle_stage_id);
    return contact;
  }
